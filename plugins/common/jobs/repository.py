class postgreSQL():
    ...
    def check_table(self,**kwargs):
        ...
        if self.schema not in schemata:
            raise RuntimeError(f"[ERROR] {self.schema} is not set yet check README file and init database..")
    ...
    def read_table(self,**kwargs):
        ...
        try:
            ...
            if table not in tables:
                raise RuntimeError(f"[INFO] {table} dose not exist.")
            ...
            print(f'[xcom_push] key : {table}, value : dataframe')
            print(f'[xcom_push] key : row_number, value : {len(df)}')

        except psycopg2.Error as e:
            print(f"[ERROR] psycopg2 error: {e}")
            raise
        except Exception as ex:
            print(f"[ERROR] runtime error: {ex}")
        finally:
            if 'cur' in locals():
    ...
    def save_to_event_table(self,**kwargs):
        ...
        print(f'[xcom_pull] key : to_save_data, value : dataframe')
