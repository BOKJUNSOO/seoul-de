<div align="center">
   <h1> 서울모아  </h1>
   <h3> 메인 서비스 테이블 구축 자동화 - Airflow Application</h3>
</div>
...
`BranchOperator` 를 이용해 서비스 DB 초기설정 혹은 데이터 갱신 부분으로 크게 2가지 flow 를 구성했습니다.
<br>
...
Airflow 컨테이너와 외부 Open AI API 와의 통신, 작성된 프롬프트를 이용한 요청을 통해 AI 요약본을 서비스에 이용할 수 있고, <br>
log를 통해 금일 갱신된 행사의 갯수, 요약본의 내용을 모니터링이 가능합니다.<br>

이는 추후 `chat-moa` 의 데이터셋으로 이용될 수 있습니다!
...
### 🎯 `MLops DAG` - data flow
작년도 월별 데이터를 DB에서 읽어오고, 일일단위로 쌓인 `feature`를 이용해 서울시 모든역의 시간대별 사용량을 예측합니다.<br>
